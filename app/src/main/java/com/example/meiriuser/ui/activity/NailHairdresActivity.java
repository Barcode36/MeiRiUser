...
import com.example.meiriuser.adapter.NailHairdresAdapter;
...
public class NailHairdresActivity extends BaseActivity{
    ...
    List<TakeOutFoodModel.DataBean> nailHairdresList;
    NailHairdresAdapter nailHairdresAdapter;
    ...
    public void initView() {
        ...
        nailHairdresList=new ArrayList<>();
        type=getIntent().getStringExtra(Constant.TYPE);
        CatIdOutsde=getIntent().getIntExtra("cat_id",0);
    ...
    public void initData() {
        ...
        nailHairdresAdapter = new NailHairdresAdapter(nailHairdresList,typeAdapterStr);
        rvType.setAdapter(nailHairdresAdapter);
        mAdapter = new TaskOutFoodClassicAdapter(mDatas);
        rvClass.setAdapter(mAdapter);
        setRefresh();
        //请求数据源
        orderCatgoryList(CatIdOutsde);
    }
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            if (result == 1) {
                                ...
                                if (bean.getData() != null) {
                                    nailHairdresList.addAll(bean.getData());
                                    nailHairdresAdapter.notifyDataSetChanged();
                                }
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
    ...
    public void initListener() {
        nailHairdresAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            @Override
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                int itemViewId = view.getId();
                switch (itemViewId) {
                    case R.id.line_item:
                        Intent intent = new Intent(NailHairdresActivity.this, GroupBuyingActivity.class);
                        intent.putExtra(Constant.TYPE, nailHairdresList.get(position).getStore_id());
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.icon_back:
                ...
                break;
            case R.id.et_search:
               /* List<AddressModel> mVals = new ArrayList<>();
                mVals.add(new AddressModel("粤保中心广场"));
                mVals.add(new AddressModel("唐人街"));
                mVals.add(new AddressModel("保利中心"));
                mVals.add(new AddressModel("湾仔路227号"));
                PreferenceUtil.saveObject("addressList", mVals);*/
                jumpToActivity(SearchActivity.class);
                break;
            case R.id.tv_address:
                Intent intent=new Intent(NailHairdresActivity.this,SearchAddressActivity.class);
